...
function extractFonts(html) {
  ...
  while ((styleTagMatch = styleTagRegex.exec(html)) !== null) {
    ...
    while ((match = lineHeightRegex.exec(styleContent)) !== null) {
      const height = match[1].trim();
      if (height && !height.includes('{') && !height.includes('}')) {
        lineHeights.set(height, (lineHeights.get(height) || 0) + 1);
      }
...
function extractSpacing(html) {
  ...
  while ((styleTagMatch = styleTagRegex.exec(html)) !== null) {
    ...
    while ((match = gapRegex.exec(styleContent)) !== null) {
      const value = match[1].trim();
      if (value && value !== '0' && !value.includes('{') && !value.includes('}')) {
        gaps.set(value, (gaps.get(value) || 0) + 1);
        spacings.set(value, (spacings.get(value) || 0) + 1);
...
function extractLayout(html) {
  ...
  while ((styleTagMatch = styleTagRegex.exec(html)) !== null) {
    ...
    let match;
    while ((match = maxWidthRegex.exec(styleContent)) !== null) {
      const value = match[1].trim();
      if (value && !value.includes('{') && !value.includes('}')) {
        layouts.maxWidth.add(value);
      }
    ...
    while ((match = displayRegex.exec(styleContent)) !== null) {
      const value = match[1].trim();
      if (value && !value.includes('{') && !value.includes('}')) {
        layouts.display.add(value);
      }
    ...
    while ((match = flexDirectionRegex.exec(styleContent)) !== null) {
      const value = match[1].trim();
      if (value && !value.includes('{') && !value.includes('}')) {
        layouts.flexDirections.add(value);
      }
    ...
    while ((match = justifyRegex.exec(styleContent)) !== null) {
      const value = match[1].trim();
      if (value && !value.includes('{') && !value.includes('}')) {
        layouts.justifies.add(value);
      }
    ...
    while ((match = alignRegex.exec(styleContent)) !== null) {
      const value = match[1].trim();
      if (value && !value.includes('{') && !value.includes('}')) {
        layouts.aligns.add(value);
      }
    ...
    while ((match = gridRegex.exec(styleContent)) !== null) {
      const value = match[1].trim();
      if (value && !value.includes('{') && !value.includes('}')) {
        layouts.grids.add(value);
      }
...
function generateMarkdown(data) {
  const { metadata, colors, fonts, spacing, components, layout, cssVariables, externalStylesheets } = data;
...
#### 字号（按使用频率排序）
${fonts.fontSizes.slice(0, 10).map((size, i) => `- **${size}**`).join('\n')}

#### 字重
${fonts.fontWeights.slice(0, 10).map(weight => `- **${weight}**`).join('\n')}

#### 行高
${fonts.lineHeights.slice(0, 10).map(height => `- **${height}**`).join('\n')}
...
### 检测到的颜色（按使用频率排序）
${colors.allColors.slice(0, 20).map((color, i) => `- **颜色${i + 1}**：${color}（使用次数：${colors.colorUsage[color]}）`).join('\n')}
...
### 间距值（按使用频率排序）
${spacing.allSpacings.slice(0, 20).map(s => `- **${s}**`).join('\n')}

### 内边距
${spacing.paddings.slice(0, 10).map(p => `- **${p}**`).join('\n')}

### 外边距
${spacing.margins.slice(0, 10).map(m => `- **${m}**`).join('\n')}

### 间距
${spacing.gaps.slice(0, 10).map(g => `- **${g}**`).join('\n')}
...
### 最大宽度
${layout.maxWidth.slice(0, 10).map(w => `- **${w}**`).join('\n')}

### 显示方式
${layout.display.slice(0, 10).map(d => `- **${d}**`).join('\n')}

### Flex 方向
${layout.flexDirections.slice(0, 10).map(fd => `- **${fd}**`).join('\n')}

### 对齐方式
${layout.justifies.slice(0, 10).map(j => `- **${j}**`).join('\n')}

### 垂直对齐
${layout.aligns.slice(0, 10).map(a => `- **${a}**`).join('\n')}

### 网格布局
${layout.grids.slice(0, 10).map(g => `- **${g}**`).join('\n')}

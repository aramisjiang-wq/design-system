...
const server = http.createServer(async (req, res) => {
  ...
  if (req.url === '/health' && req.method === 'GET') {
    ...
    res.end(JSON.stringify({ status: 'ok', message: '样式提取服务运行正常（增强版）' }));
    return;
  }
...
function extractComponents($) {
  ...
  $('button, [role="button"], .btn, .button').each((i, el) => {
    ...
    buttons.push({
      ...
      style: { ...extractStyleProperties(style), ...computedStyle }
    });
  });
...
function getComputedStyleFromElement($el) {
  const style = {};
  const el = $el[0];
